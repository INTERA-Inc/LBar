...
sns.scatterplot(x=mw3_all['Date'][5:], y=mw3_all['Diff Press'][5:], data=mw3_all, marker = '.')
...
myFmt = mdates.DateFormatter('%b-%Y')
plt.gca().xaxis.set_major_formatter(myFmt)
plt.ylim(0,60)  #updated the y-axis to range from 0 - 60
#plt.legend()  #remove legend()
plt.grid()
plt.tight_layout()
#plt.set_size_inches(8, 10)
plt.savefig("Figures/MW3_Transducer_DiffPressGraph_rev3.png",dpi=400)  #set dpi to 400 for higher resolution
